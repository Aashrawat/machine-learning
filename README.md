...
This repo contains small Python demos of **linear regression**: fitting a line with scikit-learn, drawing a bad initial guess, and then improving that guess with **gradient descent**.
...
| File | What it shows |
| --- | --- |
| `LinearRegression/linearRegression.py` | House price vs lot size with scikit-learn’s closed-form fit (`Housing.csv`). |
| `LinearRegression/linearReNoGd.py` | Synthetic data with a fixed, untrained line. |
| `LinearRegression/linearRegressionGd.py` | The same synthetic data after batch gradient descent. |
| `LinearRegression/Housing.csv` | Dataset used by the scikit-learn demo. |

## Run the demos

From the `LinearRegression` folder, with NumPy, pandas, matplotlib, and scikit-learn installed:
